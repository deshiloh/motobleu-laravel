    ...
    <form wire:submit="saveReservation" wire:loading.class="opacity-25" wire:key="form_reservation">
        <x-front.card>
            <div class="flex flex-col space-y-3">
                ...
                <div>
                    <x-toggle wire:model.live="hasBack" left-label="{{ __('Non') }}" label="{{ __('Oui') }}" md/>
                </div>
            </div>
        </x-front.card>
        <x-front.card>
            <div class="space-y-3">
                @if(!in_array(Auth::user()->entreprises()->first()->id, app(\app\Settings\BillSettings::class)->entreprise_without_command_field))
                    <x-input label="{{ __('Numéro de commande / Case code') }}" wire:model.live="reservation.commande" />
                @endif
        ...
        <x-front.card>
            <div class="space-y-3">
                ...
                @if($passagerMode == \App\Services\ReservationService::NEW_PASSAGER)
                    <div class="space-y-4">
                        <x-input label="{{ __('Nom') }} {{ __('et') }} {{ __('prénom') }}" wire:model.live="newPassager.nom"/>
                        <x-input label="{{ __('Téléphone de bureau') }}" wire:model.live="newPassager.telephone"/>
                        <x-input label="{{ __('Téléphone portable') }}" wire:model.live="newPassager.portable"/>
                        <x-input type="email" label="{{ __('Adresse email') }}" wire:model.live="newPassager.email"/>
        ...
        <x-front.card>
            <div class="space-y-3">
                ...
                @if($pickupMode == \App\Services\ReservationService::WITH_PLACE)
                    <div class="space-y-4">
                        ...
                        @if($reservation->localisation_from_id)
                            <x-input label="{{ __('Provenance / N°') }}" wire:model.live="reservation.pickup_origin" />
                        @endif
                    </div>
                @endif
                ...
                @if($pickupMode == \App\Services\ReservationService::WITH_NEW_ADRESSE)
                    <div class="space-y-4">
                        <x-input label="{{ __('Adresse') }}" wire:model.live="newAdresseReservationFrom.adresse"/>
                        <x-input label="{{ __('Adresse complémentaire') }}"
                                 wire:model.live="newAdresseReservationFrom.adresse_complement"/>
                        <x-input label="{{ __('Code postal') }}" wire:model.live="newAdresseReservationFrom.code_postal"/>
                        <x-input label="{{ __('Ville') }}" wire:model.live="newAdresseReservationFrom.ville"/>
                    </div>
                @endif
            </div>
        </x-front.card>
        ...
        <x-front.card>
            <div class="space-y-3">
                    ...
                    <div class="space-y-4">
                        ...
                        @if($reservation->localisation_to_id)
                            <div class="form-group">
                                <x-input label="{{ __('Destination / N°') }}" wire:model.live="reservation.drop_off_origin"/>
                            </div>
                        @endif
                    </div>
                @endif
                ...
                @if($dropMode == \App\Services\ReservationService::WITH_NEW_ADRESSE)
                    <div class="space-y-4">
                        <x-input label="{{ __('Adresse') }}" wire:model.live="newAdresseReservationTo.adresse"/>
                        <x-input label="{{ __('Adresse complémentaire') }}"
                                 wire:model.live="newAdresseReservationTo.adresse_complement"/>
                        <x-input label="{{ __('Code postal') }}" wire:model.live="newAdresseReservationTo.code_postal"/>
                        <x-input label="{{ __('Ville') }}" wire:model.live="newAdresseReservationTo.ville"/>
                    </div>
                @endif
            </div>
        </x-front.card>
        ...
        <x-front.card>
            <x-textarea placeholder="{{ __('Votre commentaire') }}..." wire:model.live="reservation.comment" label="{{ __('Commentaire') }}"/>
        </x-front.card>
                ...
                <div class="space-y-3">
                    ...
                    @if($backPickupMode == \App\Services\ReservationService::WITH_PLACE)
                        ...
                        @if($reservation_back->localisation_from_id)
                            <div class="form-group">
                                <x-input label="{{ __('Destination / N°') }}" wire:model.live="reservation_back.pickup_origin"/>
                            </div>
                        @endif
                    @endif
                    ...
                    @if($backPickupMode == \App\Services\ReservationService::WITH_NEW_ADRESSE)
                        <div class="space-y-4">
                            <x-input wire:model.live="newAdresseReservationFromBack.adresse" label="{{ __('Adresse') }}"/>
                            <x-input wire:model.live="newAdresseReservationFromBack.adresse_complement"
                                     label="{{ __('Adresse complémentaire') }}"/>
                            <x-input wire:model.live="newAdresseReservationFromBack.code_postal" label="{{ __('Code postal') }}"/>
                            <x-input wire:model.live="newAdresseReservationFromBack.ville" label="{{ __('Ville') }}"/>
                        </div>
                    @endif
                </div>
            </x-front.card-dark>
                ...
                <div class="space-y-3">
                    ...
                    @if($backDropMode == \App\Services\ReservationService::WITH_PLACE)
                        ...
                        @if($reservation_back->localisation_to_id)
                            <div class="form-group">
                                <x-input label="{{ __('Destination / N°') }}" wire:model.live="reservation_back.drop_off_origin"/>
                            </div>
                        @endif
                    @endif
                    ...
                    @if($backDropMode == \App\Services\ReservationService::WITH_NEW_ADRESSE)
                        <div class="space-y-4">
                            <x-input label="{{ __('Adresse') }}" wire:model.live="newAdresseReservationToBack.adresse"/>
                            <x-input label="{{ __('Adresse complémentaire') }}"
                                     wire:model.live="newAdresseReservationToBack.adresse_complement"/>
                            <x-input label="{{ __('Code postal') }}" wire:model.live="newAdresseReservationToBack.code_postal"/>
                            <x-input label="{{ __('Ville') }}" wire:model.live="newAdresseReservationToBack.ville"/>
                        </div>
                    @endif
                </div>
            </x-front.card-dark>
