                ...
                @forelse($passagers as $passager)
                    <x-datatable.tr>
                        ...
                        <x-datatable.td>
                            <div class="space-x-2">
                                @can('edit passenger')
                                    <x-mini-button icon="pencil" primary href="{{ route('front.passager.edit', ['passager' => $passager->id]) }}"/>
                                @endcan
                                @can('delete passenger')
                                    <x-mini-button icon="x-mark" red wire:click="deletePassenger({{ $passager }})"/>
                                @endcan
                            </div>
                        </x-datatable.td>
                    </x-datatable.tr>
