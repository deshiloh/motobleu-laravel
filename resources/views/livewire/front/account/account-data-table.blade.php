                ...
                @forelse($users as $user)
                    <x-datatable.tr>
                        ...
                        <x-datatable.td>
                            <div class="space-x-2">
                                @can('edit user')
                                    <x-mini-button rounded primary icon="pencil" href="{{ route('front.user.edit', ['account' => $user->id]) }}"/>
                                @endcan
                                @can('delete user')
                                    @if($user->reservations()->exists())
                                            <x-button red icon="trash" disabled label="{{ __('Suppression impossible') }}" />
                                        @else
                                            <x-mini-button rounded red icon="trash" wire:click="deleteAccountAction('{{ $user->id }}')" wire:key="{{ $user->id }}" />
                                    @endif
                                @endcan
                            </div>
                        </x-datatable.td>
